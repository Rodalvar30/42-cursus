...
/*   Updated: 2022/11/26 21:13:05 by rodalvar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	exit_draw(t_game *game, int x, int y)
{
	if (game->n_item == 0)
	{
		...
		game->img_door = mlx_xpm_file_to_image
			(game->mlx, "sprites/player/kirby9.xpm", &game->img_width, &game->img_height);
	}
	...
	while (game->map[++y])
	{
		...
		while (game->map[y][++x])
		{
			if (game->map[y][x] == '1')
				...
				img_draw(game, game->img_tile, x, y);
...
void	tombstone_draw(t_game *game)
{
	...
	game->img_player = mlx_xpm_file_to_image
		(game->mlx, "sprites/player/kirby10.xpm", &game->img_width, &game->img_height);
	game->endgame = 1;
	map_draw(game);
}
